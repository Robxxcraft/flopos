...
<div class="main w-full bg-slate-100">
  ...
  <div class="py-6 lg:py-8 px-4 md:px-8 lg:px-12">
    ...
    <div class="text-xl text-gray-800 font-bold mt-4 mb-2">Edit Category</div>
    <form class="w-full mx-auto md:px-8 py-4" @submit.prevent="updateCategory">
        <div class="flex max-w-lg mx-auto flex-wrap px-0 md:px-8  flex-col">
            ...
            <div class="flex-1 text-right">
              <button class="text-white tracking-wide bg-blue-500 py-2 px-3 rounded text-sm font-bold shadow-sm transition focus:outline-none hover:bg-blue-600 hover:shadow-lg" type="submit">Update</button>
            </div>
        </div>
    </form>
  </div>
  ...
  setup() {
    ...
    const updateCategory = () => {
      api.post(`/api/categories/${route.params.id}`, form).then(()=>{
        router.push({name: 'ManageCategory', params: {toast: 'Category has been updated'}})
      }).catch(err => {
        toast.value = true
    ...
    return { toast, toastContent, updateCategory, errors, form }
  }
};
</script>