<template>
  <div class="lg:flex min-h-screen">
      ...
      <div class="w-full px-4 md:px-8">
          ...
          <tbody>
            <template v-if="carts.length > 0">
              <TransitionGroup name="lists" :appear="true">
                <tr v-for="(cart, index) in carts" :key="cart.id" :style="`--i:${index}`">
                ...
                <td class="px-6 py-4">
                  <div class="flex items-center">
                    ...
                    <div class="basis-4/6">
                      ...
                      <div class="font-medium text-gray-500 truncate">{{cart.category}}</div>
                    </div>
                  </div>
                </td>
                ...
                <td class="text-center relative px-4">
                  <span class="border rounded px-3 py-1.5 bg-white w-24 text-gray-600">{{cart.quantity}}</span>
                  <span class="absolute right-2 cursor-pointer text-yellow-400 top-8" @click="openQuantity(cart.id, cart.quantity)">
        ...
        <div class="block lg:hidden flex flex-col space-y-4 mb-6">
          <template v-if="!skeloadingCarts">
            <div v-for="(cart, index) in carts" :key="index" class="bg-white rounded p-4">
              <div class="flex justify-between items-start">
                <div>
                  ...
                  <div class="w-48">
                    ...
                    <div class="mt-1 font-bold text-sm text-gray-500">{{cart.category}}</div>
                  </div>
                </div>
                <div title="remove item" class="btn bg-red-500 hover:bg-red-600 rounded text-white p-2 inline-block cursor-pointer" @click="removeCart(cart.id)">
                  <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="24" height="24" fill="currentColor"><path fill="none" d="M0 0h24v24H0z"/><path d="M17 6h5v2h-2v13a1 1 0 0 1-1 1H5a1 1 0 0 1-1-1V8H2V6h5V3a1 1 0 0 1 1-1h8a1 1 0 0 1 1 1v3zm1 2H6v12h12V8zm-9 3h2v6H9v-6zm4 0h2v6h-2v-6zM9 4v2h6V4H9z"/></svg>
              ...
              <div class="mt-4 flex items-end justify-between">
                ...
                <div>
                  <span class="border rounded px-3 py-1.5 bg-white w-24 text-gray-600">{{cart.quantity}}</span>
                  <span class="cursor-pointer ml-3 text-yellow-400 top-8" @click="openQuantity(cart.id, cart.quantity)">
                    <svg xmlns="http://www.w3.org/2000/svg" class="inline" viewBox="0 0 24 24" width="24" height="24" fill="currentColor"><path fill="none" d="M0 0h24v24H0z"/><path d="M15.728 9.686l-1.414-1.414L5 17.586V19h1.414l9.314-9.314zm1.414-1.414l1.414-1.414-1.414-1.414-1.414 1.414 1.414 1.414zM7.242 21H3v-4.243L16.435 3.322a1 1 0 0 1 1.414 0l2.829 2.829a1 1 0 0 1 0 1.414L7.243 21z"/></svg>
                  </span>
                  </div>
              </div>
    ...
    <div class="lg:h-screen px-4 md:px-20 lg:px-0 fixed lg:max-w-xs text-center lg:text-left w-full relative">
      ...
      <div class="mx-4 md:mx-6 lg:mx-8 mt-4">
        <div class="flex items-end justify-between pb-1 mb-3 border-b">
          ...
          <div class="text-gray-700 font-semibold text-lg">{{carts.length}}x</div>
        </div>
        <div class="flex items-end justify-between pb-1 mb-3 border-b">
          <div class="text-gray-600 font-medium">Total Quantity</div>
          <div class="text-gray-700 font-semibold text-lg">{{totalQuantity}}x</div>
        </div>
        <div class="flex items-end justify-between pb-1 mb-3 border-b">
          <div class="text-gray-600 font-medium">Total Price</div>
          <div class="text-gray-700 font-semibold text-lg">${{currencyFormat(grandTotal)}}</div>
        </div>

      </div>

      <div class="lg:absolute mt-80 lg:bottom-8 w-full px-4 md:px-8 mb-8">
      ...
      <div class="modal-bg" v-if="isQuantity">
        <div class="bg-white rounded p-6 w-full max-w-xs" ref="quantity">
          <div class="flex justify-between">
            ...
            <div class="text-red-500 cursor-pointer" @click="isQuantity = false">x</div>
          </div>
          <div class="mt-4">
...
export default {
  ...
  setup() {
    ...
    return { back, user, carts, skeloadingCarts, totalQuantity, grandTotal, openQuantity, form, changeQuantity, isModalOpen, modal, isQuantity, quantity, clearCart, removeCart, openNav, currencyFormat }
  },
}
</script>
